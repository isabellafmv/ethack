...
_SPECS = [
    ...
    _f("has_climate_oversight_committee", bool, None, "P3",
       "Named board committee with explicit climate oversight", ["S04", "S10"], snapshot=True),
    _f("comp_tied_to_emissions_target", bool, None, "P3",
       "Executive comp linked to an emissions metric", ["S04", "S10"], snapshot=True),
    _f("has_third_party_assurance", bool, None, "P3", "Emissions externally assured", ["S04", "S10"], snapshot=True),
    _f("assurance_level", str, None, "P3", "limited | reasonable | none", ["S04", "S10"], snapshot=True),
    _f("emissions_boundary_stated", bool, None, "P3",
       "Reporting boundary explicitly stated (operational/equity control)", ["S04", "S10"], snapshot=True),
    _f("has_clawback_policy", bool, None, "P3",
       "Clawback provision. WATCH NEGATION: 'we do not maintain a clawback policy' "
       "contains the keyword and means the opposite.", ["S04"], snapshot=True, snapshot=True),
    _f("has_psu_plan", bool, None, "P3", "Performance share units in the LTI plan", ["S04"], snapshot=True, snapshot=True),
    _f("performance_period_years", float, "years", "P3", "LTI performance period", ["S04"], snapshot=True),
    _f("independent_director_count", int, "count", "P3", "Independent directors", ["S04"], snapshot=True, snapshot=True),
    _f("board_size", int, "count", "P3", "Total directors", ["S04"], snapshot=True, snapshot=True),
